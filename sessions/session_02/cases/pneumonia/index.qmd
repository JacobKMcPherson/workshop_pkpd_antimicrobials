...
- **pKa**: 2.5 (acidic drug; predominantly ionized at physiological pH)
- **logP**: -1.0 (hydrophilic; limited lipid solubility)
...
**Key PK Parameters for Pneumonia**:
- **Volume of Distribution**: 0.2-0.3 L/kg (excellent lung penetration)
- **Protein Binding**: ~20% (free fraction = 0.8)
- **Renal Clearance**: ~85% unchanged in urine
- **Lung Penetration**: ELF/plasma ratio ≈ 0.6-1.0
...
<input type="text" id="drug-name-input" placeholder="Enter drug name (e.g., Cefepime)" value="Cefepime" style="width: 100%; padding: 8px; border: 1px solid #ced4da; border-radius: 4px; background-color: white;">
</div>
...
<input type="range" id="fraction-unbound-slider" min="0.0" max="1.0" value="0.8" step="0.01">
<span id="fraction-unbound-value">0.8</span>
:::

::: {.control-group-compact}
<label for="volume-slider">Volume of Distribution (L/kg):</label>
<input type="range" id="volume-slider" min="0.2" max="0.3" value="0.25" step="0.01">
<span id="volume-value">0.25</span> L/kg
:::
...
<input type="range" id="dose-slider" min="500" max="2000" value="1000" step="100">
<span id="dose-value">1000</span> mg
:::

::: {.control-group-compact}
<label for="interval-slider">Dosing Interval (hrs):</label>
<input type="range" id="interval-slider" min="8" max="12" value="8" step="1">
...
<input type="range" id="toxicity-slider" min="40" max="80" value="60" step="5">
...
<option value="active">Active transport mechanisms</option>
<option value="hydrophilic">Hydrophilic drugs access lung ELF via paracellular routes</option>
<option value="lipophilic">High lipophilicity despite negative logP</option>
<option value="metabolism">Local metabolism increases penetration</option>
</select>
...
<select id="quiz-target" style="width: 100%; padding: 5px; margin: 5px 0; border: 1px solid #ced4da; border-radius: 4px;">
<option value="">Select your answer...</option>
<option value="40">≥ 40%</option>
<option value="50">≥ 50% (T>4×MIC)</option>
<option value="70">≥ 70%</option>
<option value="100">100%</option>
</select>
<span id="quiz-target-feedback" class="quiz-feedback"></span>
:::
...
// PK Model parameters
const baseParams = {
  clearance: 7.5,      // L/h (cefepime)
  volume: 0.25,        // L/kg (cefepime volume per kg)
  dose: 1000,          // mg (typical cefepime dose)
  bioavailability: 1.0, // IV administration
  interval: 8,         // dosing interval (h) - q8h
  numDoses: 10
};
...
function generateSubjects() {
  ...
  const baseVolumePerKg = parseFloat(document.getElementById('volume-slider')?.value || 0.25);
...
function downloadData() {
  ...
  const data = {
    ...
    parameters: {
      ...
      volume: parseFloat(document.getElementById('volume-slider')?.value || 0.25),
...
function checkQuizAnswers() {
  ...
  // 6. Ionization percentage for cefepime (pKa = 2.5, pH = 7.4)
  ...
  // 7. Tissue penetration mechanism
  ...
  if (userPenetration === 'hydrophilic') {
    penetrationFeedback.textContent = '✓ Correct! Hydrophilic drugs access lung ELF via paracellular routes.';
    penetrationFeedback.className = 'quiz-feedback correct';
  } else {
    penetrationFeedback.textContent = '✗ Incorrect. Hydrophilic drugs access lung ELF via paracellular routes.';
    penetrationFeedback.className = 'quiz-feedback incorrect';
  }
  
  // 8. PK/PD target for Pseudomonas
  const userTarget = document.getElementById('quiz-target').value;
  const targetFeedback = document.getElementById('quiz-target-feedback');
  if (userTarget === '50') {
    targetFeedback.textContent = '✓ Correct! For Pseudomonas, %T>4×MIC ≥ 50% is the target.';
    targetFeedback.className = 'quiz-feedback correct';
  } else if (userTarget === '70') {
    targetFeedback.textContent = '~ Partially correct. %T>MIC ≥ 70% is general target, but %T>4×MIC ≥ 50% is preferred for Pseudomonas.';
    targetFeedback.className = 'quiz-feedback partial';
  } else {
    targetFeedback.textContent = '✗ Incorrect. For Pseudomonas, %T>4×MIC ≥ 50% is the target.';
    targetFeedback.className = 'quiz-feedback incorrect';
  }
}
...
function resetQuiz() {
  ...
  document.getElementById('quiz-target').value = '';
