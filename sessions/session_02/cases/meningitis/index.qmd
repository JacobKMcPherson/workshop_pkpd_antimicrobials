...
**Physicochemical Properties of Vancomycin**: 
- **pKa**: 7.75 (basic drug; partially ionized at physiological pH)
- **logP**: -3.1 (highly hydrophilic; very limited lipid solubility)
...
**Key PK Parameters for Meningitis**:
- **Volume of Distribution**: 0.1-0.15 L/kg (poor CNS penetration for vancomycin)
- **CSF Penetration**: Vancomycin CSF/plasma ratio ≈ 0.05-0.2 (inflamed meninges)
- **Protein Binding**: Vancomycin ~90%, Ceftriaxone ~95%
- **CNS Dosing**: Higher doses needed due to blood-brain barrier
...
<input type="text" id="drug-name-input" placeholder="Enter drug name (e.g., Vancomycin)" value="Vancomycin" style="width: 100%; padding: 8px; border: 1px solid #ced4da; border-radius: 4px; background-color: white;">
</div>
...
<input type="range" id="fraction-unbound-slider" min="0.0" max="1.0" value="0.9" step="0.01">
<span id="fraction-unbound-value">0.9</span>
:::

::: {.control-group-compact}
<label for="volume-slider">Volume of Distribution (L/kg):</label>
<input type="range" id="volume-slider" min="0.4" max="0.9" value="0.65" step="0.01">
<span id="volume-value">0.65</span> L/kg
:::

::: {.control-group-compact}
<label for="f-renal-slider">Fraction Renally Cleared:</label>
<input type="range" id="f-renal-slider" min="0.0" max="1.0" value="0.85" step="0.05">
<span id="f-renal-value">0.85</span>
:::
...
<input type="range" id="loading-dose-slider" min="0" max="3000" value="1250" step="50">
<span id="loading-dose-value">1250</span> mg
:::

::: {.control-group-compact}
<label for="dose-slider">Dose (mg):</label>
<input type="range" id="dose-slider" min="750" max="1500" value="1000" step="50">
<span id="dose-value">1000</span> mg
:::

::: {.control-group-compact}
<label for="interval-slider">Dosing Interval (hrs):</label>
<input type="range" id="interval-slider" min="8" max="12" value="12" step="1">
<span id="interval-value">12</span> hrs
:::

::: {.control-group-compact}
<label for="duration-slider">Treatment Duration (days):</label>
<input type="range" id="duration-slider" min="7" max="21" value="14" step="1">
...
<input type="range" id="toxicity-slider" min="25" max="40" value="30" step="1">
<span id="toxicity-value">20</span> mg/L
:::

::: {.control-group}
<label for="efficacy-slider">Efficacy Threshold (mg/L):</label>
<input type="range" id="efficacy-slider" min="15" max="25" value="20" step="1">
...
**Combination Therapy Rationale**: Vancomycin + ceftriaxone combination provides:
- Vancomycin: covers MRSA and resistant enterococci (though poor CNS penetration)
- Ceftriaxone: excellent CNS penetration (CSF/plasma ≈ 0.1-0.3) and covers resistant S. pneumoniae
...
<option value="inflammation">Meningeal inflammation increases blood-brain barrier permeability</option>
<option value="transport">Active transport mechanisms across BBB</option>
<option value="lipophilic">High lipophilicity despite negative logP</option>
<option value="metabolism">CNS metabolism concentrates the drug</option>
</select>
...
<select id="quiz-combination" style="width: 100%; padding: 5px; margin: 5px 0; border: 1px solid #ced4da; border-radius: 4px;">
<option value="">Select your answer...</option>
<option value="cnspenetration">Ceftriaxone provides better CNS penetration and broader coverage</option>
<option value="synergy">Direct antimicrobial synergy between the two drugs</option>
<option value="resistance">Prevents resistance development only</option>
<option value="metabolism">Different metabolic pathways reduce toxicity</option>
</select>
<span id="quiz-combination-feedback" class="quiz-feedback"></span>
:::
...
// PK Model parameters
const baseParams = {
  clearance: 4.5,      // L/h (vancomycin for meningitis)
  volume: 0.65,        // L/kg (vancomycin volume per kg)
  dose: 1000,          // mg (typical vancomycin dose for meningitis)
  bioavailability: 1.0, // IV administration
  interval: 12,        // dosing interval (h) - q12h
  numDoses: 10
};
...
function generateSubjects() {
  ...
  const baseVolumePerKg = parseFloat(document.getElementById('volume-slider')?.value || 0.65);
  ...
  const loadingDose = parseFloat(document.getElementById('loading-dose-slider')?.value || 1250);
  const dose = parseFloat(document.getElementById('dose-slider')?.value || 1000);
  const interval = parseFloat(document.getElementById('interval-slider')?.value || 12);
...
function downloadData() {
  ...
  const data = {
    ...
    parameters: {
      ...
      volume: parseFloat(document.getElementById('volume-slider')?.value || 0.65),
      bioavailability: parseFloat(document.getElementById('bioavailability-slider')?.value || 0.8),
      dose: parseFloat(document.getElementById('')?.value || 1000),
...
function resetSimulation() {
  const sliderDefaults = {
    ...
    'fraction-unbound-slider': 0.9,
    // Dosing parameters
    'loading-dose-slider': 1250,
    'dose-slider': 1000,
    'interval-slider': 12,
    'duration-slider': 10,
    // Reference lines
...
function checkQuizAnswers() {
  ...
  // 6. Vancomycin ionization percentage
  const userIonized = parseFloat(document.getElementById('quiz-ionized').value);
  const pKa = 7.75;
  const pH = 7.4; 
  const correctIonized = 100 / (1 + Math.pow(10, pH - pKa)); // For bases
  ...
  if (userPenetration === 'inflammation') {
    penetrationFeedback.textContent = '✓ Correct! Meningeal inflammation increases blood-brain barrier permeability.';
    penetrationFeedback.className = 'quiz-feedback correct';
  } else {
    penetrationFeedback.textContent = '✗ Incorrect. Meningeal inflammation increases blood-brain barrier permeability.';
    penetrationFeedback.className = 'quiz-feedback incorrect';
  }
  
  // 8. Combination therapy rationale
  const userCombination = document.getElementById('quiz-combination').value;
  const combinationFeedback = document.getElementById('quiz-combination-feedback');
  if (userCombination === 'cnspenetration') {
    combinationFeedback.textContent = '✓ Correct! Ceftriaxone provides better CNS penetration and broader coverage.';
    combinationFeedback.className = 'quiz-feedback correct';
  } else {
    combinationFeedback.textContent = '✗ Incorrect. Ceftriaxone provides better CNS penetration and broader coverage.';
    combinationFeedback.className = 'quiz-feedback incorrect';
  }
}
...
function resetQuiz() {
  ...
  document.getElementById('quiz-combination').value = '';
