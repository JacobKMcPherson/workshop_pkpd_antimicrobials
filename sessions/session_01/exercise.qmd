...
let randomSimulationData = null; // Store the random simulation for gray line
...
function runSimulation() {
  ...
  // Capture initial state for gray line (only once, when initialSimulationData is null)
...
function drawChart() {
  ...
  // Draw random simulation (incorrectly dosed patient) as light gray line
  if (randomSimulationData && randomSimulationData.subject && randomSimulationData.timePoints) {
    ctx.strokeStyle = '#cccccc'; // Light gray color for random simulation
    ctx.lineWidth = 2;
    ctx.beginPath();
    
    let firstPoint = true;
    for (let i = 0; i < randomSimulationData.timePoints.length && i < randomSimulationData.subject.concentrations.length; i++) {
      const x = margin.left + (randomSimulationData.timePoints[i] / maxTime) * width;
      const y = valueToY(randomSimulationData.subject.concentrations[i]);
...
function randomizeParameters() {
  // Use the initial simulation state for the gray line
  if (initialSimulationData) {
    randomSimulationData = {
      subject: { ...initialSimulationData.subject },
      timePoints: [...initialSimulationData.timePoints]
    };
  }
