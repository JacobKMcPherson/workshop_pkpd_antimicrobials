...
function checkQuizAnswers() {
  ...
  const updateQuizFeedback = (feedbackElement, isCorrect, correctAnswer, workedCalculation) => {
    const statusLabel = isCorrect ? '✓ Correct!' : '✗ Incorrect.';
    const workedLines = Array.isArray(workedCalculation) ? workedCalculation : [workedCalculation];
  ...
  const cssCount = Math.max(steadyStateConcentrations.length, 1);
  const cssAvgWorked = `Worked: \\(C_{ss,\\text{avg}} = \\frac{\\sum C_i}{n} = \\frac{${cssSum.toFixed(2)}}{${cssCount}} = ${correctCssAvg.toFixed(2)}\\)`;
  updateQuizFeedback(
    cssAvgFeedback,
