...
- **IBW Floor**: For instructional consistency, the ideal body weight is floored at 40 kg to avoid unrealistic values
...
function checkQuizAnswers() {
  ...
  const ibwUsed = Math.max(ibwRaw, 40); // Apply instructional IBW floor of 40 kg.
  const ibwWorked = `\\(\\text{IBW} = \\max(40, ${ibwBase.toFixed(1)} + 2.3 \\times (${heightInches.toFixed(1)} - 60)) = ${ibwUsed.toFixed(1)}\\)`;
  ...
  const vssWorked = subject.weight > 0
    ...
    : 'Worked: \\(\\text{Invalid weight value for } V_{ss,u} \\text{ calculation.}\\)';
  updateQuizFeedback(
    vssFeedback,
  ...
  const cssAvgWorked = cssCount > 0
    ...
    : `Worked (${cssRangeLabel}): \\(\\text{No concentration data available for averaging.}\\)`;
  updateQuizFeedback(
    cssAvgFeedback,
