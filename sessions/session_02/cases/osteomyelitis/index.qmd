...
- **pKa**: 2.1 (acidic drug; predominantly ionized at physiological pH)
- **logP**: -1.2 (hydrophilic; limited lipid solubility)
...
**Key PK Parameters for Osteomyelitis**:
- **Volume of Distribution**: 0.15-0.25 L/kg (moderate bone penetration)
- **Protein Binding**: ~85% (free fraction = 0.15)
- **Bone Penetration**: Bone/plasma ratio ≈ 0.1-0.2
- **Duration**: 4-6 weeks minimum for adequate bone sterilization
...
<input type="text" id="drug-name-input" placeholder="Enter drug name (e.g., Cefazolin)" value="Cefazolin" style="width: 100%; padding: 8px; border: 1px solid #ced4da; border-radius: 4px; background-color: white;">
</div>
...
<option value="medium">Medium (Standard Oral)</option>
<option value="fast" selected>Fast (IV Bolus)</option>
</select>
<span id="formulation-value">Fast (IV Bolus)</span>
:::
...
<input type="range" id="bioavailability-slider" min="0.1" max="1.0" value="1.0" step="0.05">
<span id="bioavailability-value">1.0</span>
:::

::: {.control-group-compact}
<label for="fraction-unbound-slider">Fraction Unbound:</label>
<input type="range" id="fraction-unbound-slider" min="0.0" max="1.0" value="0.15" step="0.01">
<span id="fraction-unbound-value">0.15</span>
:::

::: {.control-group-compact}
<label for="volume-slider">Volume of Distribution (L/kg):</label>
<input type="range" id="volume-slider" min="0.15" max="0.25" value="0.2" step="0.01">
<span id="volume-value">0.2</span> L/kg
:::

::: {.control-group-compact}
<label for="f-renal-slider">Fraction Renally Cleared:</label>
<input type="range" id="f-renal-slider" min="0.0" max="1.0" value="0.9" step="0.05">
<span id="f-renal-value">0.9</span>
:::
...
<input type="range" id="loading-dose-slider" min="0" max="3000" value="1500" step="50">
<span id="loading-dose-value">1500</span> mg
:::

::: {.control-group-compact}
<label for="dose-slider">Dose (mg):</label>
<input type="range" id="dose-slider" min="1000" max="2000" value="1500" step="100">
<span id="dose-value">1500</span> mg
:::

::: {.control-group-compact}
<label for="interval-slider">Dosing Interval (hrs):</label>
<input type="range" id="interval-slider" min="6" max="12" value="8" step="1">
<span id="interval-value">8</span> hrs
:::

::: {.control-group-compact}
<label for="duration-slider">Treatment Duration (days):</label>
<input type="range" id="duration-slider" min="28" max="42" value="42" step="1">
...
<input type="range" id="toxicity-slider" min="100" max="200" value="150" step="10">
<span id="toxicity-value">20</span> mg/L
:::

::: {.control-group}
<label for="efficacy-slider">Efficacy Threshold (mg/L):</label>
<input type="range" id="efficacy-slider" min="8" max="32" value="16" step="2">
...
**Prolonged Treatment Requirements**: Osteomyelitis requires 4-6 weeks minimum due to:
- Poor vascularization of infected bone
- Biofilm formation by Staphylococcus species  
- Need for complete bone sterilization
- Prevention of chronic infection development
...
::: {.quiz-question}
**6. Cefazolin PK/PD: Based on cefazolin's pKa (2.1) and physiological pH (7.4), what percentage of the drug is ionized?**
<input type="number" id="quiz-ionized" placeholder="Enter percentage (0-100)" step="0.1" style="width: 100%; padding: 5px; margin: 5px 0; border: 1px solid #ced4da; border-radius: 4px;">
<span id="quiz-ionized-feedback" class="quiz-feedback"></span>
<div class="quiz-hint" style="font-size: 0.9em; color: #666; margin-top: 5px;">
...
<select id="quiz-penetration" style="width: 100%; padding: 5px; margin: 5px 0; border: 1px solid #ced4da; border-radius: 4px;">
<option value="">Select your answer...</option>
<option value="extracellular">Hydrophilic distribution via extracellular fluid and bone tissue affinity</option>
<option value="transport">Active transport mechanisms into bone</option>
<option value="lipophilic">Actually has high lipophilicity despite negative logP</option>
<option value="metabolism">Local bone metabolism concentrates the drug</option>
</select>
<span id="quiz-penetration-feedback" class="quiz-feedback"></span>
:::

::: {.quiz-question}
**8. Treatment Duration: Why does osteomyelitis typically require 4-6 weeks of antibiotic therapy?**
<select id="quiz-duration" style="width: 100%; padding: 5px; margin: 5px 0; border: 1px solid #ced4da; border-radius: 4px;">
<option value="">Select your answer...</option>
<option value="sterilization">Extended duration needed for adequate bone sterilization and biofilm penetration</option>
<option value="resistance">Prevents development of antibiotic resistance</option>
<option value="absorption">Poor oral absorption requires longer treatment</option>
<option value="toxicity">Lower doses require longer duration to avoid toxicity</option>
</select>
<span id="quiz-duration-feedback" class="quiz-feedback"></span>
:::
...
// PK Model parameters
const baseParams = {
  clearance: 3.5,      // L/h (cefazolin)
  volume: 0.2,         // L/kg (cefazolin volume per kg)
  dose: 1500,          // mg (typical cefazolin dose)
  bioavailability: 1.0, // IV administration
  interval: 8,         // dosing interval (h) - typical q8h
  numDoses: 10
};
...
function generateSubjects() {
  ...
  const baseVolumePerKg = parseFloat(document.getElementById('volume-slider')?.value || 0.2);
  const bioavailability = parseFloat(document.getElementById('bioavailability-slider')?.value || 1.0);
  const fractionUnbound = parseFloat(document.getElementById('fraction-unbound-slider')?.value || 0.15);
  ...
  const loadingDose = parseFloat(document.getElementById('loading-dose-slider')?.value || 1500);
  const dose = parseFloat(document.getElementById('dose-slider')?.value || 1500);
  const interval = parseFloat(document.getElementById('interval-slider')?.value || 8);
...
function downloadData() {
  ...
  const data = {
    ...
    parameters: {
      fRenal: parseFloat(document.getElementById('f-renal-slider')?.value || 0.9),
      volume: parseFloat(document.getElementById('volume-slider')?.value || 0.2),
      bioavailability: parseFloat(document.getElementById('bioavailability-slider')?.value || 1.0),
      dose: parseFloat(document.getElementById('')?.value || 1500),
      interval: parseFloat(document.getElementById('interval-slider')?.value || 8),
...
function resetSimulation() {
  const sliderDefaults = {
    ...
    'fraction-unbound-slider': 0.15,
    // Dosing parameters
    'loading-dose-slider': 1500,
    'dose-slider': 1500,
    'interval-slider': 8,
    'duration-slider': 10,
    // Reference lines
...
function checkQuizAnswers() {
  ...
  // 6. Cefazolin ionization percentage
  const userIonized = parseFloat(document.getElementById('quiz-ionized').value);
  const pKa = 2.1;
  const pH = 7.4;
  const correctIonized = 100 * Math.pow(10, pH - pKa) / (1 + Math.pow(10, pH - pKa));
  const ionizedFeedback = document.getElementById('quiz-ionized-feedback');
  if (Math.abs(userIonized - correctIonized) / correctIonized <= 0.05) { // 5% tolerance for percentage
    ionizedFeedback.textContent = `✓ Correct! (${correctIonized.toFixed(1)}%)`;
    ionizedFeedback.className = 'quiz-feedback correct';
  } else {
    ionizedFeedback.textContent = `✗ Incorrect. Correct answer: ${correctIonized.toFixed(1)}%`;
    ionizedFeedback.className = 'quiz-feedback incorrect';
  }
  
  // 7. Bone penetration mechanism
  const userPenetration = document.getElementById('quiz-penetration').value;
  const penetrationFeedback = document.getElementById('quiz-penetration-feedback');
  if (userPenetration === 'extracellular') {
    penetrationFeedback.textContent = '✓ Correct! Hydrophilic distribution via extracellular fluid and bone tissue affinity.';
    penetrationFeedback.className = 'quiz-feedback correct';
  } else {
    penetrationFeedback.textContent = '✗ Incorrect. Hydrophilic distribution via extracellular fluid and bone tissue affinity.';
    penetrationFeedback.className = 'quiz-feedback incorrect';
  }
  
  // 8. Treatment duration rationale
  const userDuration = document.getElementById('quiz-duration').value;
  const durationFeedback = document.getElementById('quiz-duration-feedback');
  if (userDuration === 'sterilization') {
    durationFeedback.textContent = '✓ Correct! Extended duration needed for adequate bone sterilization and biofilm penetration.';
    durationFeedback.className = 'quiz-feedback correct';
  } else {
    durationFeedback.textContent = '✗ Incorrect. Extended duration needed for adequate bone sterilization and biofilm penetration.';
    durationFeedback.className = 'quiz-feedback incorrect';
  }
}
...
function resetQuiz() {
  ...
  document.getElementById('quiz-ionized').value = '';
  document.getElementById('quiz-penetration').value = '';
