...
- **IBW Floor**: For instructional consistency, the ideal body weight is floored at 40 kg to prevent unrealistic calculations when the formula yields values below physiologic norms
...
function checkQuizAnswers() {
  ...
  const vssWorked = subject.weight > 0
    ...
    : 'Worked: \\(\\text{Weight must be greater than 0 for } V_{ss,u} \\text{ calculation.}\\)';
  updateQuizFeedback(
    vssFeedback,
  ...
  if (window.MathJax?.typesetPromise) {
    MathJax.typesetPromise(feedbackElements)
      .catch(err => console.warn('MathJax failed to render quiz feedback equations; LaTeX source will appear instead of formatted formulas.', err));
  }
}
