...
<input type="range" id="pka-slider" min="2" max="12" value="7.4" step="0.2">
<span id="pka-value">7.4</span>
:::

::: {.control-group}
<label for="logp-slider">LogP (Lipophilicity):</label>
<input type="range" id="logp-slider" min="-2" max="6" value="2.5" step="0.1">
<span id="logp-value">2.5</span>
:::

::: {.control-group}
<label for="fu-slider">Protein Binding (fu):</label>
<input type="range" id="fu-slider" min="0.01" max="1.0" value="0.1" step="0.01">
<span id="fu-value">0.1</span>
:::
...
::: {.control-group}
<label for="creatinine-slider">Creatinine Clearance (mL/min):</label>
<input type="range" id="creatinine-slider" min="30" max="150" value="120" step="5">
<span id="creatinine-value">120</span> mL/min
:::

::: {.control-group}
<label for="albumin-slider">Albumin (g/L):</label>
<input type="range" id="albumin-slider" min="20" max="50" value="40" step="2">
<span id="albumin-value">40</span> g/L
:::

::: {.control-group}
<label for="pregnancy-select">Pregnancy Status:</label>
<select id="pregnancy-select">
...
.drug-properties, .patient-properties, .dosing-properties {
  display: grid;
...
function generateSubjects() {
  ...
  const pka = parseFloat(document.getElementById('pka-slider')?.value || 7.4);
  const logp = parseFloat(document.getElementById('logp-slider')?.value || 2.5);
  const fu = parseFloat(document.getElementById('fu-slider')?.value || 0.1);
  ...
  const creatinine_cl = parseFloat(document.getElementById('creatinine-slider')?.value || 120);
  const albumin = parseFloat(document.getElementById('albumin-slider')?.value || 40);
  const pregnancy = document.getElementById('pregnancy-select')?.value || 'none';
  ...
  const renal_factor = creatinine_cl / 120; // Normalized to normal CrCl of 120 mL/min
  
  // Protein binding adjustment (lower albumin = higher free fraction)
  const protein_factor = (40 / albumin); // Normalized to normal albumin of 40 g/L
  const adjusted_fu = Math.min(1.0, fu * protein_factor);
  
  // Pregnancy adjustments (simplified)
  let pregnancy_factor = 1.0;
  if (pregnancy === 'second' || pregnancy === 'third') {
    pregnancy_factor = 1.3; // Increased clearance in 2nd/3rd trimester
  }
  
  // Calculate final clearance
  const base_clearance = 5.0; // L/h baseline
  const clearance = base_clearance * age_factor * sex_factor * renal_factor * pregnancy_factor * adjusted_fu;
  
  // Calculate effective bioavailability (affected by fa and first-pass metabolism)
  const effective_bioavailability = fa * bioavailability;
...
function calculateConcentration(subject, time) {
  const { clearance, volume, dose, bioavailability, interval } = subject;
  ...
  for (let n = 0; n < baseParams.numDoses; n++) {
...
function updateSliderValues() {
  const updates = [
    ...
    { slider: 'fu-slider', display: 'fu-value' },
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const sliderIds = [
    'dose-slider', 'pka-slider', 'logp-slider', 'fu-slider', 'vd-slider', 
    'fa-slider', 'bioavailability-slider', 'age-slider', 'weight-slider', 
    'height-slider', 'creatinine-slider', 'albumin-slider', 'interval-slider'
  ];
  ...
  const selectIds = ['sex-select', 'pregnancy-select'];
