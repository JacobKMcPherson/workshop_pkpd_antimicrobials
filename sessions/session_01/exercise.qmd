...
function generateSubjects() {
  ...
  // Calculate patient-specific clearance using Cockcroft-Gault equation
  ...
  // Convert creatinine clearance from mL/min to L/h for consistency with clearance units
  creatinineClearance = creatinineClearance * 0.06; // mL/min to L/h conversion
  
  // Use creatinine clearance to adjust the base clearance
  // Assuming a typical creatinine clearance of ~120 mL/min (7.2 L/h) for normalization
  const normalCrCl = 7.2; // L/h
  const clearance = baseClearance * (creatinineClearance / normalCrCl);
...
function drawChart() {
  ...
  ctx.fillText('Plasma Concentration-Time Profile (Blood Samples Every Hour)', canvas.width/2, 25);
...
function updateTable() {
  ...
  subjects.forEach(subject => {
    ...
    row.innerHTML = `
      ...
      <td>${subject.creatinineClearance ? subject.creatinineClearance.toFixed(2) : 'N/A'}</td>
      <td>${subject.dose}</td>
...
function randomizeParameters() {
  ...
  Object.entries(randomParams).forEach(([id, range]) => {
    ...
    if (element) {
      ...
      let finalValue;
      if (id === 'clearance-slider' || id === 'bioavailability-slider' || id === 'volume-slider') {
