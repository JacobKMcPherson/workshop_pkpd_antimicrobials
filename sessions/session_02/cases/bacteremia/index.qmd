...
<input type="range" id="dose-slider" min="400" max="800" value="750" step="50">
<span id="dose-value">750</span> mg
:::

::: {.control-group-compact}
<label for="interval-slider">Dosing Interval (hrs):</label>
<input type="range" id="interval-slider" min="1" max="48" value="12" step="1">
...
- **MIC Breakpoint**: MRSA susceptibility defined as MIC ≤ 2 mg/L
- **Heteroresistance**: Some MRSA strains show subpopulations with higher MICs
- **Biofilm Formation**: MRSA in bacteremia may form biofilms requiring higher exposures
- **Source Control**: Removal of infected devices critical for treatment success

**Dosing Strategy for MRSA Bacteremia**:
- **Loading Dose**: 25-30 mg/kg to rapidly achieve target concentrations
- **Maintenance**: 15-20 mg/kg q8-12h based on renal function
- **Monitoring**: Target AUC₀₋₂₄ = 400-600 mg⋅h/L
- **Duration**: Minimum 2 weeks, longer for complicated infections
...
**6. Vancomycin PK/PD: Based on vancomycin's pKa (7.75) and physiological pH (7.4), what percentage of the drug is unionized?**
</div>
<input type="number" id="quiz-unionized" placeholder="Enter percentage (0-100)" step="0.1" style="width: 100%; padding: 5px; margin: 5px 0; border: 1px solid #ced4da; border-radius: 4px;">
<span id="quiz-unionized-feedback" class="quiz-feedback"></span>
...
<select id="quiz-distribution" style="width: 100%; padding: 5px; margin: 5px 0; border: 1px solid #ced4da; border-radius: 4px;">
<option value="">Select your answer...</option>
<option value="hydrophilic">Very low logP (-3.1) and large molecular size limit membrane penetration</option>
<option value="protein">High protein binding prevents distribution</option>
<option value="active">Active efflux pumps remove it from cells</option>
<option value="metabolism">Rapid metabolism prevents tissue accumulation</option>
</select>
<span id="quiz-distribution-feedback" class="quiz-feedback"></span>
:::

::: {.quiz-question}
**8. PK/PD Target: What is the optimal AUC/MIC target for vancomycin against MRSA?**
<select id="quiz-auc-mic" style="width: 100%; padding: 5px; margin: 5px 0; border: 1px solid #ced4da; border-radius: 4px;">
<option value="">Select your answer...</option>
...
<span id="quiz-auc-mic-feedback" class="quiz-feedback"></span>
:::
...
// PK Model parameters
const baseParams = {
  clearance: 4.5,      // L/h (vancomycin)
  volume: 0.65,        // L/kg (vancomycin volume per kg)
  dose: 750,           // mg (typical vancomycin maintenance dose)
  bioavailability: 1.0, // IV administration
  interval: 12,        // dosing interval (h) - typical q12h
  numDoses: 10
};
...
function generateSubjects() {
  ...
  const dose = parseFloat(document.getElementById('dose-slider')?.value || 750);
...
function downloadData() {
  ...
  const data = {
    ...
    parameters: {
      ...
      dose: parseFloat(document.getElementById('')?.value || 750),
...
function checkQuizAnswers() {
  ...
  // 6. Unionized percentage for vancomycin (pKa = 7.75, pH = 7.4)
  const userUnionized = parseFloat(document.getElementById('quiz-unionized').value);
  const pKa = 7.75;
  const pH = 7.4;
  const correctUnionized = 100 / (1 + Math.pow(10, pH - pKa));
  const unionizedFeedback = document.getElementById('quiz-unionized-feedback');
  if (Math.abs(userUnionized - correctUnionized) / correctUnionized <= 0.05) {
    unionizedFeedback.textContent = `✓ Correct! (${correctUnionized.toFixed(1)}%)`;
    unionizedFeedback.className = 'quiz-feedback correct';
  } else {
    unionizedFeedback.textContent = `✗ Incorrect. Correct answer: ${correctUnionized.toFixed(1)}%`;
    unionizedFeedback.className = 'quiz-feedback incorrect';
  }
  
  // 7. Distribution mechanism
  const userDistribution = document.getElementById('quiz-distribution').value;
  const distributionFeedback = document.getElementById('quiz-distribution-feedback');
  if (userDistribution === 'hydrophilic') {
    distributionFeedback.textContent = '✓ Correct! Very low logP and large size limit membrane penetration.';
    distributionFeedback.className = 'quiz-feedback correct';
  } else {
    distributionFeedback.textContent = '✗ Incorrect. Very low logP (-3.1) and large molecular size limit membrane penetration.';
    distributionFeedback.className = 'quiz-feedback incorrect';
  }
  
  // 8. AUC/MIC target
  const userAucMic = document.getElementById('quiz-auc-mic').value;
  const aucMicFeedback = document.getElementById('quiz-auc-mic-feedback');
  if (userAucMic === '400') {
    aucMicFeedback.textContent = '✓ Correct! AUC/MIC ≥ 400-600 is the target for MRSA.';
    aucMicFeedback.className = 'quiz-feedback correct';
  } else {
    aucMicFeedback.textContent = '✗ Incorrect. AUC/MIC ≥ 400-600 is the target for MRSA.';
    aucMicFeedback.className = 'quiz-feedback incorrect';
  }
}
...
function resetQuiz() {
  ...
  document.getElementById('quiz-unionized').value = '';
  document.getElementById('quiz-distribution').value = '';
  document.getElementById('quiz-auc-mic').value = '';
