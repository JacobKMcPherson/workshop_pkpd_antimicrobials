...
// Two-compartment model calculation
function calculateConcentrationTwoCompartment(subject, time) {
  ...
  // Note: Two-compartment models with non-first-order kinetics are mathematically complex
  // For now, we only support first-order kinetics for two-compartment models
  const kineticsType = kinetics || 'first-order';
  if (kineticsType !== 'first-order') {
    console.warn('Two-compartment model currently only supports first-order kinetics. Using first-order.');
  }
