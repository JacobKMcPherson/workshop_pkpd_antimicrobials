...
function checkQuizAnswers() {
  ...
  const updateQuizFeedback = (feedbackElement, isCorrect, correctAnswer, workedCalculation) => {
    ...
    feedbackElement.innerHTML = `${statusLabel} Correct answer: ${correctAnswer}<div class="quiz-feedback-detail">${workedCalculation}</div>`;
  ...
  updateQuizFeedback(
    ...
    `Worked: ${ibwWorked}<br>${adjBwWorked}`
  );
  ...
  const cssStartIdx = timePoints.findIndex(t => t >= 216);
  const cssEndIdx = timePoints.findIndex(t => t >= 240);
