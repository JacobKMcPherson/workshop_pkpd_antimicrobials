...
<div class="controls-column-1">
#### Patient Generator
<div class="patient-generator">
<div style="margin-bottom: 8px;">
<button id="generate-patient-btn" onclick="generateRandomPatient()">New Patient</button>
<div id="patient-info" style="margin-top: 8px;">
...
<label for="pk-model-select" style="margin-top: 10px; display: block;">PK Model:</label>
<select id="pk-model-select" style="width: 100%; padding: 5px; border: 1px solid #ced4da; border-radius: 4px; background-color: white;" onchange="handleModelChange()">
<option value="one-compartment" selected>One-Compartment</option>
<option value="two-compartment">Two-Compartment</option>
<option value="nca">Non-Compartmental Analysis (NCA)</option>
</select>

<label for="kinetics-select" style="margin-top: 10px; display: block;">Elimination Kinetics:</label>
<select id="kinetics-select" style="width: 100%; padding: 5px; border: 1px solid #ced4da; border-radius: 4px; background-color: white;" onchange="runSimulation()">
...
<option value="non-linear">Non-Linear (Michaelis-Menten)</option>
</select>
</div>

</div>

<div class="controls-column-2">
#### PK Parameters
<div class="pk-properties-compact">
<div class="small-buttons" style="margin-top: 2px;">
...
<input type="range" id="bioavailability-slider" min="0" max="1.0" value="1.0" step="0.05">
<span id="bioavailability-value">1.0</span>
</div>

<div class="control-group-compact">
<label for="fraction-unbound-slider">Fraction Unbound:</label>
<input type="range" id="fraction-unbound-slider" min="0" max="1.0" value="1.0" step="0.05">
<span id="fraction-unbound-value">1.0</span>
</div>

<div class="control-group-compact">
<label for="volume-slider">Volume of Distribution (L/kg):</label>
<input type="range" id="volume-slider" min="0.1" max="10.00" value="0.70" step="0.1">
<span id="volume-value">0.7</span> L/kg
</div>

<div id="two-compartment-params" style="display: none;">
<div class="control-group-compact">
<label for="v2-slider">Peripheral Volume V2 (L/kg):</label>
<input type="range" id="v2-slider" min="0.1" max="10" value="0.5" step="0.1">
<span id="v2-value">0.5</span> L/kg
</div>

<div class="control-group-compact">
<label for="q-slider">Intercompartmental Clearance Q (L/h):</label>
<input type="range" id="q-slider" min="0.5" max="10.0" value="2.0" step="0.5">
<span id="q-value">2.0</span> L/h
</div>
</div>

<div class="control-group-compact">
<label for="f-renal-slider">Fraction Renally Cleared:</label>
<input type="range" id="f-renal-slider" min="0.0" max="1.0" value="1.0" step="0.05">
<span id="f-renal-value">1.0</span>
</div>

</div>
</div>

<div class="controls-column-3">
#### Dosing
<div class="pk-properties-compact">
<button id="download-btn" onclick="downloadData()">Download Data</button>
<button id="save-patient-btn" onclick="saveCurrentPatient()" style="background-color: #28a745; color: white; margin-top: 5px; width: 100%;">Save Patient</button>
...
<div class="control-group-compact">
<label for="toxicity-slider">Toxicity (mg/L):</label>
<input type="range" id="toxicity-slider" min="0.000" max="100" value="20" step="1">
<span id="toxicity-value">20</span> mg/L
</div>

<div class="control-group-compact">
<label for="efficacy-slider">Efficacy (mg/L):</label>
<input type="range" id="efficacy-slider" min="0" max="100" value="2" step="0.5">
...
<div id="concentration-plot">
<div id="chart-fallback">
  <p>Loading interactive chart...</p>
  <canvas id="pk-chart" width="1920" height="1080"></canvas>
</div>
...
- **Creatinine Clearance**: Calculated using Cockcroft-Gault:
  \[
  \mathrm{CrCl} = \frac{(140 - \text{age}) \times \text{weight} \times (0.85\ \text{if female})}{72 \times S_{\text{Cr}}}
  \]
- **Weight-Based Dosing**: Volume of distribution scales directly with patient weight
- **IBW Floor**: For instructional consistency, the ideal body weight is floored at 40 kg to prevent
  unrealistic calculations when the formula yields values below physiologic norms
...
- **Peripheral Volume (V2)**: Volume of the peripheral compartment (L/kg)
- **Intercompartmental Clearance (Q)**: Rate of drug transfer between central and peripheral compartments (L/h)

This model is particularly useful for drugs that:
- Show multi-phasic elimination (initial rapid decline followed by slower terminal phase)
- Have extensive tissue distribution (e.g., lipophilic drugs)
- Require loading doses to account for peripheral distribution

**Two-Compartment Equations:**
- Central compartment:
  \[
  \frac{dA_1}{dt} = -k_{10} A_1 - k_{12} A_1 + k_{21} A_2 + k_a A_{\text{abs}}
  \]
- Peripheral compartment:
  \[
  \frac{dA_2}{dt} = k_{12} A_1 - k_{21} A_2
  \]
- Where: \(k_{10} = \frac{CL}{V_1},\ k_{12} = \frac{Q}{V_1},\ k_{21} = \frac{Q}{V_2}\)
...
**Key NCA Parameters:**
- **AUC (Area Under the Curve)**: Total drug exposure calculated using the trapezoidal rule
- **Cmax**: Maximum observed concentration
- **Tmax**: Time to maximum concentration
- **Terminal Half-life**: Calculated from the terminal elimination phase (regression of log-concentration vs time)
- **Clearance (CL/F)**:
  \[
  \frac{CL}{F} = \frac{\text{Dose}}{\text{AUC}}
  \]

**Advantages of NCA:**
- No model assumptions required
- Robust to sparse sampling
- Regulatory gold standard for bioequivalence studies
- Less prone to bias than compartmental methods

##### Elimination Kinetics

This simulation supports multiple elimination kinetic models:

**First-Order Kinetics (Default):**
- Elimination rate proportional to drug concentration:
  \[
  \frac{dC}{dt} = -k_e C
  \]
- Constant half-life independent of dose
- Most common for therapeutic drugs
...
- Constant elimination rate regardless of concentration:
  ...
  \frac{dC}{dt} = -k_0
  \]
- Seen when elimination pathways are saturated
- Examples: Alcohol, phenytoin at high doses
- Non-linear pharmacokinetics

**Second-Order Kinetics:**
- Elimination rate proportional to concentration squared:
  \[
  \frac{dC}{dt} = -k_2 C^2
  \]
- Faster elimination at higher concentrations
- Rare in clinical practice

**Third-Order Kinetics:**
- Elimination rate proportional to concentration cubed:
  \[
  \frac{dC}{dt} = -k_3 C^3
  \]
- Even faster elimination at higher concentrations
- Theoretical interest, rarely observed clinically
...
- Saturable elimination:
  ...
  \]
- Combination of first-order (low C) and zero-order (high C)
- Examples: Phenytoin, salicylates, theophylline at high doses
...
\text{IBW} =
\begin{cases}
50 + 2.3(\text{Ht}_{\text{in}} - 60) & \text{male} \\
45.5 + 2.3(\text{Ht}_{\text{in}} - 60) & \text{female}
\end{cases}
\]
\[
\text{AdjBW} =
\begin{cases}
\text{IBW} + 0.4(\text{Wt} - \text{IBW}) & \text{if Wt} > \text{IBW} \\
\text{Wt} & \text{if Wt} \le \text{IBW}
\end{cases}
\]
</div>
...
<div class="quiz-equation">
\[
V_{ss,u} = f_u \times V = f_u \times (V_{d/kg} \times \text{Wt})
\]
</div>
...
<div class="quiz-equation">
\[
C_{ss,\text{avg}} = \frac{\sum_{i=1}^{n} C_i}{n}
\]
</div>
...
.controls-column-4 h3,
.controls-column-1 h4,
.controls-column-2 h4,
.controls-column-3 h4,
.controls-column-4 h4 {
  margin: 0 0 15px 0;
  color: #495057;
  font-size: 1.1em;
  border-bottom: 2px solid #007bff;
...
.control-group-compact {
  ...
  gap: 0px; /* Remove vertical gap between elements */
  padding: 4px; /* Less padding for a narrower look */
  background-color: #fff;
  border-radius: 4px;
  border: 1px solid #dee2e6;
}

/* Make sure the value span has minimal vertical margin */
.control-group-compact span {
  font-size: 0.9em;
  color: #6c757d;
  text-align: center;
  margin-bottom: 0;       /* Remove extra space below */
  margin-top: 1px;        /* Minimal space above */
...
// Global variables
let subjects = [];
let savedPatients = []; // Array to store all saved patients
let subjectCounter = 0; // Counter for subject IDs
let timePoints = [];
let canvas = null;
let ctx = null;
let isLogScale = true; // Track current scale type
let initialSimulationData = null; // Store the initial simulation state
...
// Handle PK model selection change
function handleModelChange() {
  const pkModel = document.getElementById('pk-model-select')?.value || 'one-compartment';
  const twoCompartmentParams = document.getElementById('two-compartment-params');
  const pkModelValue = document.getElementById('pk-model-value');

  // Show/hide two-compartment specific parameters
  if (twoCompartmentParams) {
    twoCompartmentParams.style.display = pkModel === 'two-compartment' ? 'block' : 'none';
  }
  ...
  if (pkModelValue) {
    ...
    pkModelValue.textContent = modelNames[pkModel] || 'One-Compartment';
  }
...
// NCA real-time Plot and Data for a concentration-time profile
function calculateNCAParameters(concentrations, timePoints, interval) {
  // Find Cmax and Tmax
  let cmax = 0;
  let tmax = 0;
  ...
  // Calculate AUC using trapezoidal rule for steady-state (last dosing interval)
  // Find the start of the last dosing interval
  const lastDoseStartTime = Math.floor(timePoints[timePoints.length - 1] / interval) * interval;
  const lastDoseEndTime = lastDoseStartTime + interval;

  let auc = 0;
  for (let i = 1; i < concentrations.length; i++) {
    const t1 = timePoints[i - 1];
    const t2 = timePoints[i];
    const c1 = concentrations[i - 1];
    const c2 = concentrations[i];

    if (t1 >= lastDoseStartTime && t2 <= lastDoseEndTime) {
      // Trapezoidal rule: AUC = (C1 + C2) / 2 * (T2 - T1)
      auc += (c1 + c2) / 2 * (t2 - t1);
    }
  }

  // Calculate terminal half-life using the last 3-5 points
  // Find terminal elimination phase (log-linear regression)
  const TERMINAL_PHASE_POINTS = 10; // Number of points to use for terminal phase selection
  const terminalPoints = [];
  const startTerminalIdx = Math.max(0, concentrations.length - TERMINAL_PHASE_POINTS);

  for (let i = startTerminalIdx; i < concentrations.length; i++) {
    if (concentrations[i] > 0) {
      terminalPoints.push({
        time: timePoints[i],
        logConc: Math.log(concentrations[i])
      });
    }
  }
  ...
  const n = terminalPoints.length;

  for (let point of terminalPoints) {
    sumX += point.time;
    sumY += point.logConc;
    sumXY += point.time * point.logConc;
    sumX2 += point.time * point.time;
  }

  const slope = n > 1 ? (n * sumXY - sumX * sumY) / (n * sumX2 - sumX * sumX) : 0;
  const lambda_z = -slope; // Terminal elimination rate constant
  const terminalHalfLife = lambda_z > 0 ? 0.693 / lambda_z : 0;
...
function generateSubjects() {
  ...
  const fRenal = parseFloat(document.getElementById('f-renal-slider')?.value || 1.0);
  const baseVolumePerKg = parseFloat(document.getElementById('volume-slider')?.value || 0.7);
  const bioavailability = parseFloat(document.getElementById('bioavailability-slider')?.value || 1.0);
  const fractionUnbound = parseFloat(document.getElementById('fraction-unbound-slider')?.value || 1.0);
  ...
  // Two-compartment specific parameters
  const v2PerKg = parseFloat(document.getElementById('v2-slider')?.value || 0.5);
  const q = parseFloat(document.getElementById('q-slider')?.value || 2.0);

  // PK model and kinetics selection
  const pkModel = document.getElementById('pk-model-select')?.value || 'one-compartment';
  const kinetics = document.getElementById('kinetics-select')?.value || 'first-order';
  ...
  subjects.push({
    id: currentPatient.subjectId || 'temp',
    // Drug information
    drugName: drugName,
    // PK model and kinetics
    pkModel: pkModel,
    kinetics: kinetics,
...
function generateRandomPatient() {
  ...
  // Update global patient data with subject ID
  currentPatient = {
    subjectId: subjectCounter,
    age,
    sex,
    height,
    weight,
...
// Save current patient with dosing strategy to the Data tab
function saveCurrentPatient() {
  if (!currentPatient.subjectId) {
  ...
  // Check if this patient is already saved
  const existingPatientIndex = savedPatients.findIndex(p => p.id === currentPatient.subjectId);

  if (existingPatientIndex !== -1) {
    // Update existing patient
    savedPatients[existingPatientIndex] = { ...currentSubject, id: currentPatient.subjectId };
    alert(`Patient ${currentPatient.subjectId} dosing strategy updated!`);
  } else {
    // Add new patient
    savedPatients.push({ ...currentSubject, id: currentPatient.subjectId });
    alert(`Patient ${currentPatient.subjectId} saved successfully!`);
  }

  // Update the data table to show all saved patients
  updateTable();
}

// Helper function to calculate concentration based on real-time Plot and Data
// For one-compartment model with first-order absorption
function calculateConcentrationWithKinetics(dose, bioavailability, vssUnbound, ka, ke, time, kinetics) {
  // Absorption phase - always first-order
  const absorbedFraction = 1 - Math.exp(-ka * time);
  const amountAbsorbed = dose * bioavailability * absorbedFraction;

  // Initial concentration after absorption (assuming instantaneous distribution)
  const C0 = (dose * bioavailability * ka) / vssUnbound;

  // Calculate concentration based on elimination kinetics
  switch (kinetics) {
    case 'first-order':
      // Standard first-order: C = C0 * (exp(-ke*t) - exp(-ka*t)) / (ka - ke)
      return C0 * (Math.exp(-ke * time) - Math.exp(-ka * time)) / (ka - ke);

    case 'zero-order':
      // Zero-order: constant elimination rate until drug is depleted
      // During absorption: amount increases, during elimination: linear decline

      if (time === 0) return 0;

      // Amount absorbed up to time t
      const amountAbsorbed_t = dose * bioavailability * (1 - Math.exp(-ka * time));

      // Zero-order elimination rate (constant mg/h)
      const k0 = ke * vssUnbound; // mg/h

      // Amount eliminated up to time t (integral of k0 from 0 to t)
      const amountEliminated = k0 * time;

      // Net amount in body
      const amountInBody = Math.max(0, amountAbsorbed_t - amountEliminated);

      // Concentration
      return amountInBody / vssUnbound;

    case 'second-order':
      // Second-order: dC/dt = -k2 * C^2
      // This requires numerical integration due to simultaneous absorption and elimination
      let C_second = 0;
      const dt_second = 0.1; // time step
      const steps_second = Math.floor(time / dt_second);
      const k2 = ke / 10; // Scaled second-order rate constant

      for (let i = 0; i < steps_second; i++) {
        const t = i * dt_second;
        // Absorption rate at time t
        const dCabs_dt = (dose * bioavailability * ka / vssUnbound) * Math.exp(-ka * t);
        // Second-order elimination rate
        const dCelim_dt = k2 * C_second * C_second;
        // Net change
        C_second += (dCabs_dt - dCelim_dt) * dt_second;
        C_second = Math.max(0, C_second);
      }

      return C_second;

    case 'third-order':
      // Third-order: dC/dt = -k3 * C^3
      // Requires numerical integration
      let C_third = 0;
      const dt_third = 0.1; // time step
      const steps_third = Math.floor(time / dt_third);
      const k3 = ke / 100; // Scaled third-order rate constant

      for (let i = 0; i < steps_third; i++) {
        const t = i * dt_third;
        // Absorption rate at time t
        const dCabs_dt = (dose * bioavailability * ka / vssUnbound) * Math.exp(-ka * t);
        // Third-order elimination rate
        const dCelim_dt = k3 * C_third * C_third * C_third;
        // Net change
        C_third += (dCabs_dt - dCelim_dt) * dt_third;
        C_third = Math.max(0, C_third);
      }
    ...
    case 'non-linear':
      ...
      // Approximate using numerical integration (Euler method)
      // Vmax and Km are derived from ke and typical concentrations
      const Vmax = ke * vssUnbound * 10; // Maximum elimination rate (mg/h)
      const Km = 5; // Michaelis constant (mg/L) - can be adjusted

      // Start with initial concentration after absorption
      let C = 0;
      const dt = 0.1; // time step for numerical integration
      ...
      for (let i = 0; i < steps; i++) {
        ...
        // Absorption rate
        const dCabs_dt = (dose * bioavailability * ka / vssUnbound) * Math.exp(-ka * t);
        // Elimination rate (Michaelis-Menten)
        const dCelim_dt = (Vmax * C) / (Km + C) / vssUnbound;
        // Net change
        C += (dCabs_dt - dCelim_dt) * dt;
        C = Math.max(0, C); // Concentration cannot be negative
      }
...
function calculateConcentration(subject, time) {
  ...
  if (pkModel === 'two-compartment') {
    return calculateConcentrationTwoCompartment(subject, time);
  } else if (pkModel === 'nca') {
    return calculateConcentrationNCA(subject, time);
  } else {
    return calculateConcentrationOneCompartment(subject, time);
  }
}

// One-compartment model calculation
function calculateConcentrationOneCompartment(subject, time) {
  const { clearance, vssUnbound, dose, bioavailability, interval, numDoses, loadingDose, kinetics } = subject;
  ...
  const tau = interval;
  const kineticsType = kinetics || 'first-order';

  let concentration = 0;

  // Apply loading dose at time 0
  if (loadingDose > 0 && time >= 0) {
    const conc = calculateConcentrationWithKinetics(
      loadingDose, bioavailability, vssUnbound, ka, ke, time, kineticsType
    );
    concentration += conc;
  }

  // Apply maintenance doses from the second dose onward
  for (let n = 1; n < numDoses; n++) {
    const doseTime = n * tau;
    if (time >= doseTime) {
      const timeSinceDose = time - doseTime;
      const conc = calculateConcentrationWithKinetics(
        dose, bioavailability, vssUnbound, ka, ke, timeSinceDose, kineticsType
      );
      concentration += conc;
    }
  }
...
// Two-compartment model calculation with support for different elimination kinetics
function calculateConcentrationTwoCompartment(subject, time) {
  const { clearance, vssUnbound, dose, bioavailability, interval, numDoses, loadingDose, v2PerKg, q, weight, kinetics } = subject;
  ...
  const v1 = vssUnbound; // Central compartment volume
  const v2 = v2PerKg * weight; // Peripheral compartment volume

  // Micro-constants
  const k10 = clearance / v1;
  const k12 = q / v1;
  const k21 = q / v2;

  const tau = interval;
  
  // For first-order kinetics, use analytical solution
  if (kineticsType === 'first-order') {
    // Macro-constants (hybrid rate constants)
    ...
    // Apply loading dose at time 0
    if (loadingDose > 0 && time >= 0) {
      const A = (loadingDose * bioavailability / v1) * (ka - alpha) / ((ka - alpha) * (beta - alpha));
      const B = (loadingDose * bioavailability / v1) * (ka - beta) / ((ka - beta) * (alpha - beta));
      const C = (loadingDose * bioavailability / v1) * ((alpha - ka) * (beta - ka)) / ((ka - alpha) * (ka - beta));

      const conc = A * Math.exp(-alpha * time) + B * Math.exp(-beta * time) + C * Math.exp(-ka * time);
      concentration += conc;
    }
    ...
    for (let n = 1; n < numDoses; n++) {
      ...
      if (time >= doseTime) {
        ...
        const A = (dose * bioavailability / v1) * (ka - alpha) / ((ka - alpha) * (beta - alpha));
        const B = (dose * bioavailability / v1) * (ka - beta) / ((ka - beta) * (alpha - beta));
        const C = (dose * bioavailability / v1) * ((alpha - ka) * (beta - ka)) / ((ka - alpha) * (ka - beta));

        const conc = A * Math.exp(-alpha * timeSinceDose) + B * Math.exp(-beta * timeSinceDose) + C * Math.exp(-ka * timeSinceDose);
        concentration += conc;
      }
    }
    ...
    // For non-first-order kinetics, use numerical integration
    return calculateConcentrationTwoCompartmentNumerical(subject, time, kineticsType, ka, v1, v2, k10, k12, k21);
  }
}

// Numerical integration for two-compartment model with non-first-order kinetics
function calculateConcentrationTwoCompartmentNumerical(subject, time, kineticsType, ka, v1, v2, k10, k12, k21) {
  const { dose, bioavailability, interval, numDoses, loadingDose } = subject;
  
  // Initialize compartment amounts
  let C1 = 0; // Central compartment concentration
  let C2 = 0; // Peripheral compartment concentration
  let absorbedAmount = 0; // Amount absorbed so far
  
  const dt = 0.1; // Time step for numerical integration
  const tau = interval;
  
  // Simulate from time 0 to target time
  const steps = Math.floor(time / dt);
  
  for (let i = 0; i < steps; i++) {
    const t = i * dt;
    
    // Determine which dose(s) are being absorbed at this time
    let totalAbsorptionRate = 0;
    
    // Loading dose (at time 0)
    if (loadingDose > 0 && t >= 0) {
      const timeSinceLoadingDose = t;
      totalAbsorptionRate += (loadingDose * bioavailability * ka) * Math.exp(-ka * timeSinceLoadingDose);
    }
    ...
    for (let n = 1; n < numDoses; n++) {
      ...
      if (t >= doseTime) {
        ...
        totalAbsorptionRate += (dose * bioavailability * ka) * Math.exp(-ka * timeSinceDose);
      }
    }
    
    // Absorption into central compartment
    const dC1_abs = totalAbsorptionRate / v1;
    
    // Distribution between compartments (always first-order)
    const dC1_dist = -k12 * C1 + k21 * C2;
    const dC2_dist = k12 * C1 - k21 * C2;
    
    // Elimination from central compartment (kinetics-dependent)
    let dC1_elim = 0;
    
    switch (kineticsType) {
      case 'zero-order':
        // Zero-order: constant elimination rate
        const k0_rate = k10 * v1; // Convert to amount/time
        dC1_elim = k0_rate / v1; // Convert back to concentration/time
        break;
        
      case 'second-order':
        // Second-order: dC/dt = -k2 * C^2
        const k2 = k10 / 10; // Scaled second-order rate constant
        dC1_elim = k2 * C1 * C1;
        break;
        
      case 'third-order':
        // Third-order: dC/dt = -k3 * C^3
        const k3 = k10 / 100; // Scaled third-order rate constant
        dC1_elim = k3 * C1 * C1 * C1;
        break;
        
      case 'non-linear':
        // Michaelis-Menten kinetics
        const Vmax = k10 * v1 * 10; // Maximum elimination rate
        const Km = 5; // Michaelis constant
        dC1_elim = (Vmax * C1) / (Km + C1) / v1;
        break;
        
      default:
        // Default to first-order (shouldn't reach here)
        dC1_elim = k10 * C1;
    }
    
    // Update concentrations
    C1 += (dC1_abs + dC1_dist - dC1_elim) * dt;
    C2 += dC2_dist * dt;
  ...
  // Return central compartment concentration (what we measure)
  return C1;
}

// NCA-based concentration calculation (uses one-compartment as basis, then applies real-time Plot and Data)
function calculateConcentrationNCA(subject, time) {
  return calculateConcentrationOneCompartment(subject, time);
}
...
function runSimulation() {
  ...
  subjects.forEach(subject => {
    ...
    if (startIdx !== -1 && endIdx !== -1) {
      ...
      // Find Cmax and Tmax in steady-state period
      let maxConc = 0;
      let maxTime = 0;
      for (let i = startIdx; i < endIdx; i++) {
        if (subject.concentrations[i] > maxConc) {
          maxConc = subject.concentrations[i];
          maxTime = timePoints[i];
        }
      }
      subject.cmax = maxConc;
      subject.tmax = maxTime - 216; // Relative to start of steady-state interval
    }
    ...
    // For NCA model, calculate additional NCA-specific parameters
    if (subject.pkModel === 'nca') {
      const ncaParams = calculateNCAParameters(subject.concentrations, timePoints, subject.interval);
      subject.ncaHalfLife = ncaParams.terminalHalfLife;
      subject.ncaCmax = ncaParams.cmax;
      subject.ncaTmax = ncaParams.tmax;
      subject.ncaAUC = ncaParams.auc;
    }
  });
  ...
  // Capture initial state (only once, when initialSimulationData is null)
  if (initialSimulationData === null && subjects.length > 0) {
...
function drawChart() {
  ...
  const kineticsNames = {
    ...
    'non-linear': 'Non-Linear (Michaelis-Menten)'
  };
  const kineticsLabel = kineticsNames[kinetics] || 'First-Order';

  ctx.fillText(`Plasma Concentration-Time Profile (${scaleType}, ${kineticsLabel} Kinetics)`, canvas.width/2, 25);
...
function updateTable() {
  ...
  // Show all saved patients first
  savedPatients.forEach(subject => {
    const row = tbody.insertRow();
    row.innerHTML = `
      <td>${subject.id}</td>
      <td>${subject.drugName || 'N/A'}</td>
      <td>${subject.pkModel || 'one-compartment'}</td>
      <td>${subject.kinetics || 'first-order'}</td>
      <td>${subject.age}</td>
      <td>${subject.sex}</td>
      ...
      <td>${subject.tmax ? subject.tmax.toFixed(1) : 'N/A'}</td>
  ...
  // Then show current simulation if exists (highlighted in blue)
  subjects.forEach(subject => {
    const row = tbody.insertRow();
    row.style.backgroundColor = '#e3f2fd';
    row.innerHTML = `
      <td>${subject.id} (Current)</td>
      <td>${subject.drugName || 'N/A'}</td>
      <td>${subject.pkModel || 'one-compartment'}</td>
      <td>${subject.kinetics || 'first-order'}</td>
      <td>${subject.age}</td>
...
function downloadPlotImage() {
  ...
  link.download = `pk_simulation_plot_${new Date().toISOString().split('T')[0]}.svg`;
...
function resetSimulation() {
  const sliderDefaults = {
    ...
    'f-renal-slider': 1.0,
    'volume-slider': 0.7,
    'bioavailability-slider': 1.0,
    'fraction-unbound-slider': 1.0,
    // Two-compartment parameters
    'v2-slider': 0.5,
    'q-slider': 2.0,
    // Dosing parameters
  ...
  // Reset PK Model and Elimination Kinetics to defaults
  const pkModelSelect = document.getElementById('pk-model-select');
  if (pkModelSelect) {
    pkModelSelect.value = 'one-compartment';
    handleModelChange();
  }

  const kineticsSelect = document.getElementById('kinetics-select');
  if (kineticsSelect) {
    kineticsSelect.value = 'first-order';
  }
...
function randomizeParameters() {
  ...
  const randomParams = {
    ...
    'bioavailability-slider': { min: 0, max: 1.0 },
    'fraction-unbound-slider': { min: 0, max: 1.0 },
    // Dosing parameters
  ...
  const pkModelSelect = document.getElementById('pk-model-select');
  if (pkModelSelect) {
    const pkModels = ['one-compartment', 'two-compartment', 'nca'];
    const randomModel = pkModels[Math.floor(Math.random() * pkModels.length)];
    pkModelSelect.value = randomModel;
    handleModelChange();
  }

  // Randomize Elimination Kinetics
  const kineticsSelect = document.getElementById('kinetics-select');
  if (kineticsSelect) {
    const kineticsOptions = ['zero-order', 'first-order', 'second-order', 'third-order', 'non-linear'];
    const randomKinetics = kineticsOptions[Math.floor(Math.random() * kineticsOptions.length)];
    kineticsSelect.value = randomKinetics;
  }
...
function checkQuizAnswers() {
  ...
  const INSTRUCTIONAL_IBW_FLOOR_KG = 40;
  // workedCalculationLines accepts a single string or an array for multi-line feedback.
  const updateQuizFeedback = (feedbackElement, isCorrect, correctAnswer, workedCalculationLines) => {
    const statusLabel = isCorrect ? '✓ Correct!' : '✗ Incorrect.';
    const workedLines = Array.isArray(workedCalculationLines)
      ? workedCalculationLines
      : [workedCalculationLines];
    feedbackElement.replaceChildren();
    const statusLine = document.createElement('div');
    statusLine.textContent = `${statusLabel} Correct answer: ${correctAnswer}`;
    feedbackElement.appendChild(statusLine);
    workedLines.forEach((line, index) => {
      const detailLine = document.createElement('div');
      detailLine.className = 'quiz-feedback-detail';
      detailLine.textContent = line;
      if (index > 0) {
        detailLine.style.marginTop = '2px';
      }
      feedbackElement.appendChild(detailLine);
    });
    feedbackElement.className = `quiz-feedback ${isCorrect ? 'correct' : 'incorrect'}`;
  };
  // Steady state evaluated from 216-240 hours (from day 9 to day 10).
  const STEADY_STATE_START_HOURS = 216;
  const STEADY_STATE_END_HOURS = 240;
  ...
  const crClFactor = subject.sex === 'Female' ? 0.85 : 1;
  const crClWorked = `Worked: \\(\\frac{(140 - ${subject.age}) \\times ${subject.weight.toFixed(1)} \\times ${crClFactor}}{72 \\times ${subject.creatinine.toFixed(2)}} = ${correctCrCl.toFixed(1)}\\)`;
  updateQuizFeedback(
    crClFeedback,
    Math.abs(userCrCl - correctCrCl) / correctCrCl <= tolerance,
    `${correctCrCl.toFixed(1)} mL/min`,
    crClWorked
  );
  ...
  const heightInches = subject.height / 2.54;
  const ibwBase = subject.sex === 'Male' ? 50 : 45.5;
  const ibwRaw = ibwBase + 2.3 * (heightInches - 60);
  const ibwUsed = Math.max(ibwRaw, INSTRUCTIONAL_IBW_FLOOR_KG); // Apply instructional IBW floor.
  const ibwWorked = `\\(\\text{IBW} = \\max(${INSTRUCTIONAL_IBW_FLOOR_KG}, ${ibwBase.toFixed(1)} + 2.3 \\times (${heightInches.toFixed(1)} - 60)) = ${ibwUsed.toFixed(1)}\\)`;
  const adjBwWorked = subject.weight > ibwUsed
    ? `\\(\\text{AdjBW} = ${ibwUsed.toFixed(1)} + 0.4 \\times (${subject.weight.toFixed(1)} - ${ibwUsed.toFixed(1)}) = ${correctAdjBW.toFixed(1)}\\)`
    : `\\(\\text{AdjBW} = ${subject.weight.toFixed(1)}\\)`;
  updateQuizFeedback(
    adjBWFeedback,
    Math.abs(userAdjBW - correctAdjBW) / correctAdjBW <= tolerance,
    `${correctAdjBW.toFixed(1)} kg`,
    [`Worked: ${ibwWorked}`, adjBwWorked]
  );
  ...
  const vssWorked = subject.weight > 0
    ? `Worked: \\(V = ${(subject.volume / subject.weight).toFixed(2)} \\times ${subject.weight.toFixed(1)} = ${subject.volume.toFixed(1)},\\ V_{ss,u} = ${subject.fractionUnbound.toFixed(2)} \\times ${subject.volume.toFixed(1)} = ${correctVss.toFixed(1)}\\)`
    : `Worked: \\(\\text{Weight must be > 0 (current: ${subject.weight.toFixed(1)} kg) `
      + `for } V_{ss,u} \\text{ calculation.}\\)`;
  updateQuizFeedback(
    vssFeedback,
    Math.abs(userVss - correctVss) / correctVss <= tolerance,
    `${correctVss.toFixed(1)} L`,
    vssWorked
  );
  ...
  const tssWorked = `Worked: \\(t_{ss} = 5 \\times ${subject.halfLife.toFixed(1)} = ${correctTss.toFixed(1)}\\)`;
  updateQuizFeedback(
    tssFeedback,
    Math.abs(userTss - correctTss) / correctTss <= tolerance,
    `${correctTss.toFixed(1)} hours`,
    tssWorked
  );
  ...
  const cssStartIdx = timePoints.findIndex(t => t >= STEADY_STATE_START_HOURS);
  const cssEndIdx = timePoints.findIndex(t => t >= STEADY_STATE_END_HOURS);
  const hasSteadyStateWindow = cssStartIdx !== -1 && cssEndIdx !== -1;
  const steadyStateConcentrations = hasSteadyStateWindow
    ? subject.concentrations.slice(cssStartIdx, cssEndIdx)
    : subject.concentrations;
  const cssSum = steadyStateConcentrations.reduce((sum, conc) => sum + conc, 0);
  const cssCount = steadyStateConcentrations.length;
  const cssRangeLabel = hasSteadyStateWindow
    ? `${STEADY_STATE_START_HOURS}-${STEADY_STATE_END_HOURS} h`
    : 'full profile';
  const cssAvgWorked = cssCount > 0
    ? `Worked (${cssRangeLabel}): \\(C_{ss,\\text{avg}} = \\frac{\\sum C_i}{n} = \\frac{${cssSum.toFixed(2)}}{${cssCount}} = ${correctCssAvg.toFixed(2)}\\)`
    : `Worked (${cssRangeLabel}): \\(\\text{No concentration data available in the expected time range for averaging.}\\)`;
  updateQuizFeedback(
    cssAvgFeedback,
    Math.abs(userCssAvg - correctCssAvg) / correctCssAvg <= tolerance,
    `${correctCssAvg.toFixed(2)} mg/L`,
    cssAvgWorked
  );

  const feedbackElements = [crClFeedback, adjBWFeedback, vssFeedback, tssFeedback, cssAvgFeedback];
  if (window.MathJax?.typesetPromise) {
    const mathJaxWarning = `MathJax failed to render quiz feedback equations; LaTeX source will appear instead of formatted formulas.`;
    MathJax.typesetPromise(feedbackElements)
      .catch(err => console.warn(mathJaxWarning, err));
  }
}
...
function updateSliderValues() {
  const updates = [
    ...
    // Two-compartment parameters
    { slider: 'v2-slider', display: 'v2-value' },
    { slider: 'q-slider', display: 'q-value' },
    // Dosing parameters
  ...
  // Special handling for kinetics select
  const kineticsSelect = document.getElementById('kinetics-select');
  const kineticsDisplay = document.getElementById('kinetics-value');
  if (kineticsSelect && kineticsDisplay) {
    const selectedOption = kineticsSelect.options[kineticsSelect.selectedIndex];
    kineticsDisplay.textContent = selectedOption.text;
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (savedPatient) {
    ...
    // Update subject counter if there's a saved subject ID
    if (currentPatient.subjectId) {
      subjectCounter = Math.max(subjectCounter, currentPatient.subjectId);
    }
  } else {
    // No saved patient, so we need to generate one initially
    generateRandomPatient();
    return; // generateRandomPatient will handle the rest of initialization
  }
...
[🧪 pH-Dependent Drug Distribution Exercise →](exercise_logd.qmd){.btn .btn-outline-primary}
