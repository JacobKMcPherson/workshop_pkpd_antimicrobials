...
function calculateConcentrationWithKinetics(dose, bioavailability, vssUnbound, ka, ke, time, kinetics) {
  ...
  switch (kinetics) {
    ...
    case 'zero-order':
      ...
      // Rate of elimination: k0 = ke * C0 * V (constant amount/time)
      const k0 = ke * vssUnbound; // mg/h elimination rate
      const initialAmount = dose * bioavailability;
      
      // Concentration after absorption phase
      if (time === 0) return 0;
      const concAfterAbsorption = (dose * bioavailability * ka / vssUnbound) * (1 - Math.exp(-ka * time)) / ka;
      const amountAfterAbsorption = concAfterAbsorption * vssUnbound;
      
      // Time when drug would be fully eliminated
      const eliminationTime = amountAfterAbsorption / k0;
      
      // Linear decline
      if (time < eliminationTime) {
        return Math.max(0, concAfterAbsorption - (k0 / vssUnbound) * time);
      } else {
        return 0;
      }
    ...
    case 'second-order':
      ...
      // Solution: C = C0 / (1 + C0*k2*t)
      const concAbsorbed = (dose * bioavailability / vssUnbound) * (1 - Math.exp(-ka * time));
      const k2 = ke / vssUnbound; // second-order rate constant
      if (concAbsorbed === 0) return 0;
      return concAbsorbed / (1 + concAbsorbed * k2 * time);
      
    case 'third-order':
      // Third-order: dC/dt = -k3 * C^3
      // Solution: C = C0 / sqrt(1 + 2*C0^2*k3*t)
      const concAbs3 = (dose * bioavailability / vssUnbound) * (1 - Math.exp(-ka * time));
      const k3 = ke / (vssUnbound * vssUnbound); // third-order rate constant
      if (concAbs3 === 0) return 0;
      return concAbs3 / Math.sqrt(1 + 2 * concAbs3 * concAbs3 * k3 * time);
