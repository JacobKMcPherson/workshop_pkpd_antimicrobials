...
- **logP**: -0.5 (moderately hydrophilic with some lipophilic character)
...
**Key PK Parameters for UTI**:
- **Volume of Distribution**: 0.1-0.2 L/kg (excellent urinary concentration)
- **Protein Binding**: ~95% (free fraction = 0.05)
- **Renal Clearance**: ~90% unchanged in urine
- **Urine Concentrations**: 10-100× higher than plasma levels
...
<input type="text" id="drug-name-input" placeholder="Enter drug name (e.g., Ertapenem)" value="Ertapenem" style="width: 100%; padding: 8px; border: 1px solid #ced4da; border-radius: 4px; background-color: white;">
</div>
...
<input type="range" id="fraction-unbound-slider" min="0.0" max="1.0" value="0.05" step="0.01">
<span id="fraction-unbound-value">0.05</span>
:::

::: {.control-group-compact}
<label for="volume-slider">Volume of Distribution (L/kg):</label>
<input type="range" id="volume-slider" min="0.1" max="0.2" value="0.15" step="0.01">
<span id="volume-value">0.15</span> L/kg
:::
...
<input type="range" id="loading-dose-slider" min="0" max="3000" value="1000" step="50">
<span id="loading-dose-value">1000</span> mg
:::

::: {.control-group-compact}
<label for="dose-slider">Dose (mg):</label>
<input type="range" id="dose-slider" min="500" max="1000" value="1000" step="50">
...
<input type="range" id="toxicity-slider" min="30" max="60" value="40" step="5">
<span id="toxicity-value">20</span> mg/L
:::

::: {.control-group}
<label for="efficacy-slider">Efficacy Threshold (mg/L):</label>
<input type="range" id="efficacy-slider" min="1" max="8" value="4" step="0.5">
...
<option value="renal">High renal clearance (90%) concentrates drug 10-100× higher in urine than plasma</option>
<option value="lipophilic">High lipophilicity enables tissue penetration</option>
<option value="transport">Active transport mechanisms into urinary tract</option>
<option value="metabolism">Local urinary metabolism concentrates the drug</option>
</select>
...
<select id="quiz-coverage" style="width: 100%; padding: 5px; margin: 5px 0; border: 1px solid #ced4da; border-radius: 4px;">
<option value="">Select your answer...</option>
<option value="stable">β-lactamase stability and carbapenem structure resistant to ESBL hydrolysis</option>
<option value="binding">Higher protein binding increases effectiveness</option>
<option value="ph">pH-dependent activity optimized for urinary tract</option>
<option value="absorption">Better oral absorption than other options</option>
</select>
<span id="quiz-coverage-feedback" class="quiz-feedback"></span>
:::
...
// PK Model parameters
const baseParams = {
  clearance: 1.5,      // L/h (ertapenem)
  volume: 0.15,        // L/kg (ertapenem volume per kg)
  dose: 1000,          // mg (typical ertapenem dose)
...
function generateSubjects() {
  ...
  const baseVolumePerKg = parseFloat(document.getElementById('volume-slider')?.value || 0.15);
  ...
  const loadingDose = parseFloat(document.getElementById('loading-dose-slider')?.value || 1000);
...
function downloadData() {
  ...
  const data = {
    ...
    parameters: {
      ...
      volume: parseFloat(document.getElementById('volume-slider')?.value || 0.15),
...
function resetSimulation() {
  const sliderDefaults = {
    ...
    'fraction-unbound-slider': 0.05,
    // Dosing parameters
    'loading-dose-slider': 1000,
    'dose-slider': 1000,
    'interval-slider': 24,
    'duration-slider': 10,
    // Reference lines
...
function checkQuizAnswers() {
  ...
  // 6. Ertapenem ionization percentage
  ...
  if (userPenetration === 'renal') {
    penetrationFeedback.textContent = '✓ Correct! High renal clearance (90%) concentrates drug 10-100× higher in urine than plasma.';
    penetrationFeedback.className = 'quiz-feedback correct';
  } else {
    penetrationFeedback.textContent = '✗ Incorrect. High renal clearance (90%) concentrates drug 10-100× higher in urine than plasma.';
    penetrationFeedback.className = 'quiz-feedback incorrect';
  }
  
  // 8. ESBL coverage mechanism
  const userCoverage = document.getElementById('quiz-coverage').value;
  const coverageFeedback = document.getElementById('quiz-coverage-feedback');
  if (userCoverage === 'stable') {
    coverageFeedback.textContent = '✓ Correct! β-lactamase stability and carbapenem structure resistant to ESBL hydrolysis.';
    coverageFeedback.className = 'quiz-feedback correct';
  } else {
    coverageFeedback.textContent = '✗ Incorrect. β-lactamase stability and carbapenem structure resistant to ESBL hydrolysis.';
    coverageFeedback.className = 'quiz-feedback incorrect';
  }
}
...
function resetQuiz() {
  ...
  document.getElementById('quiz-coverage').value = '';
