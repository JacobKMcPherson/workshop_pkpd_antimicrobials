...
<span id="pk-model-value">One-Compartment</span>
...
<input type="range" id="volume-slider" min="0.4" max="0.9" value="0.7" step="0.1">
...
<input type="range" id="toxicity-slider" min="15" max="30" value="20" step="1">
<span id="toxicity-value">20</span> mg/L
:::

::: {.control-group}
<label for="efficacy-slider">Efficacy Threshold (mg/L):</label>
<input type="range" id="efficacy-slider" min="10" max="20" value="15" step="0.5">
...
function calculateNCAParameters(concentrations, timePoints, interval) {
  ...
  const startTerminalIdx = Math.max(0, concentrations.length - 10);
...
function calculateConcentrationTwoCompartment(subject, time) {
  const { clearance, vssUnbound, dose, bioavailability, interval, numDoses, loadingDose, v2PerKg, q, weight } = subject;
...
function calculateConcentrationNCA(subject, time) {
  // For NCA, we still need to generate concentration data first
  // We'll use the one-compartment model as the underlying data source
  // The NCA calculations will be performed on this data
  return calculateConcentrationOneCompartment(subject, time);
}
