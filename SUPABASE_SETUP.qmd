...
If survey forms are not working: 1. Verify that both secrets are correctly set in repository settings 2. Check the GitHub Actions logs for any errors during the "Inject Supabase secrets" step 3. Ensure the secret values don't contain special characters that might interfere with the substitution process
