...
function checkQuizAnswers() {
  ...
  const volumePerKg = subject.weight > 0 ? subject.volume / subject.weight : 0;
  const vssWorked = `Worked: \\(V = ${volumePerKg.toFixed(2)} \\times ${subject.weight.toFixed(1)} = ${subject.volume.toFixed(1)},\\ V_{ss,u} = ${subject.fractionUnbound.toFixed(2)} \\times ${subject.volume.toFixed(1)} = ${correctVss.toFixed(1)}\\)`;
  updateQuizFeedback(
    vssFeedback,
  ...
  const steadyStateConcentrations = (cssStartIdx !== -1 && cssEndIdx !== -1)
  ...
  const cssAvgWorked = cssCount > 0
    ? `Worked: \\(C_{ss,\\text{avg}} = \\frac{\\sum C_i}{n} = \\frac{${cssSum.toFixed(2)}}{${cssCount}} = ${correctCssAvg.toFixed(2)}\\)`
    : 'Worked: steady-state window unavailable for averaging.';
  updateQuizFeedback(
    cssAvgFeedback,
  ...
  if (window.MathJax?.typesetPromise) {
    MathJax.typesetPromise([crClFeedback, adjBWFeedback, vssFeedback, tssFeedback, cssAvgFeedback])
      .catch(err => console.warn('MathJax failed to render quiz feedback equations:', err));
  }
}
