...
- **pKa**: 2.8 (acidic drug; predominantly ionized at physiological pH)
- **logP**: -0.1 (moderately hydrophilic with some lipophilic character)
...
**Key PK Parameters for Endocarditis**:
- **Volume of Distribution**: 0.1 L/kg (poor tissue penetration)
- **Protein Binding**: ~8% (high free fraction = 0.92)
- **Cardiac Tissue**: Limited penetration, requires high systemic exposure
- **Dosing**: 8-10 mg/kg/day for complex endocarditis
...
<input type="text" id="drug-name-input" placeholder="Enter drug name (e.g., Vancomycin)" value="Daptomycin" style="width: 100%; padding: 8px; border: 1px solid #ced4da; border-radius: 4px; background-color: white;">
</div>
...
<input type="range" id="fraction-unbound-slider" min="0.0" max="1.0" value="0.08" step="0.01">
<span id="fraction-unbound-value">0.08</span>
:::

::: {.control-group-compact}
<label for="volume-slider">Volume of Distribution (L/kg):</label>
<input type="range" id="volume-slider" min="0.08" max="0.12" value="0.1" step="0.01">
<span id="volume-value">0.1</span> L/kg
:::

::: {.control-group-compact}
<label for="f-renal-slider">Fraction Renally Cleared:</label>
<input type="range" id="f-renal-slider" min="0.0" max="1.0" value="0.5" step="0.05">
<span id="f-renal-value">0.5</span>
:::
...
<input type="range" id="loading-dose-slider" min="0" max="3000" value="700" step="50">
<span id="loading-dose-value">700</span> mg
:::

::: {.control-group-compact}
<label for="dose-slider">Dose (mg):</label>
<input type="range" id="dose-slider" min="400" max="800" value="700" step="50">
<span id="dose-value">700</span> mg
:::
...
<input type="range" id="duration-slider" min="14" max="42" value="28" step="1">
...
<input type="range" id="toxicity-slider" min="50" max="100" value="75" step="5">
<span id="toxicity-value">20</span> mg/L
:::

::: {.control-group}
<label for="efficacy-slider">Efficacy Threshold (mg/L):</label>
<input type="range" id="efficacy-slider" min="20" max="35" value="25" step="1">
...
- **CPK levels**: Monitor for myopathy (dose-related toxicity)
- **Pulmonary function**: Rare but serious eosinophilic pneumonia
- **Renal function**: Potential nephrotoxicity with high doses
...
<option value="penetration">Limited cardiac tissue penetration requires high systemic exposure</option>
<option value="resistance">Higher doses prevent VRE resistance development</option>
<option value="binding">High protein binding reduces free drug availability</option>
<option value="metabolism">Rapid cardiac metabolism requires higher doses</option>
</select>
...
<select id="quiz-mechanism" style="width: 100%; padding: 5px; margin: 5px 0; border: 1px solid #ced4da; border-radius: 4px;">
<option value="">Select your answer...</option>
<option value="calcium">Calcium-dependent mechanism requires adequate tissue calcium concentrations</option>
<option value="ph">pH-dependent activity requires specific tissue pH</option>
<option value="oxygen">Oxygen-dependent mechanism limits anaerobic effectiveness</option>
<option value="temperature">Temperature-sensitive mechanism affects distribution</option>
</select>
<span id="quiz-mechanism-feedback" class="quiz-feedback"></span>
:::
...
// PK Model parameters
const baseParams = {
  clearance: 1.0,      // L/h (daptomycin)
  volume: 0.1,         // L/kg (daptomycin volume per kg)
  dose: 700,           // mg (typical daptomycin dose for endocarditis)
  bioavailability: 1.0, // IV administration
  interval: 24,        // dosing interval (h) - q24h
  numDoses: 10
};
...
function generateSubjects() {
  ...
  const baseVolumePerKg = parseFloat(document.getElementById('volume-slider')?.value || 0.1);
  ...
  const loadingDose = parseFloat(document.getElementById('loading-dose-slider')?.value || 700);
  const dose = parseFloat(document.getElementById('dose-slider')?.value || 700);
...
function downloadData() {
  ...
  const data = {
    ...
    parameters: {
      ...
      volume: parseFloat(document.getElementById('volume-slider')?.value || 0.1),
      bioavailability: parseFloat(document.getElementById('bioavailability-slider')?.value || 0.8),
      dose: parseFloat(document.getElementById('')?.value || 700),
...
function resetSimulation() {
  const sliderDefaults = {
    ...
    'fraction-unbound-slider': 0.08,
    // Dosing parameters
    'loading-dose-slider': 700,
    'dose-slider': 700,
    'interval-slider': 24,
    'duration-slider': 10,
    // Reference lines
...
function checkQuizAnswers() {
  ...
  // 6. Daptomycin ionization percentage
  ...
  // 7. Cardiac penetration and dosing
  ...
  if (userPenetration === 'penetration') {
    penetrationFeedback.textContent = '✓ Correct! Limited cardiac tissue penetration requires high systemic exposure.';
    penetrationFeedback.className = 'quiz-feedback correct';
  } else {
    penetrationFeedback.textContent = '✗ Incorrect. Limited cardiac tissue penetration requires high systemic exposure.';
    penetrationFeedback.className = 'quiz-feedback incorrect';
  }
  
  // 8. Calcium-dependent mechanism
  const userMechanism = document.getElementById('quiz-mechanism').value;
  const mechanismFeedback = document.getElementById('quiz-mechanism-feedback');
  if (userMechanism === 'calcium') {
    mechanismFeedback.textContent = '✓ Correct! Calcium-dependent mechanism requires adequate tissue calcium concentrations.';
    mechanismFeedback.className = 'quiz-feedback correct';
  } else {
    mechanismFeedback.textContent = '✗ Incorrect. Calcium-dependent mechanism requires adequate tissue calcium concentrations.';
    mechanismFeedback.className = 'quiz-feedback incorrect';
  }
}
...
function resetQuiz() {
  ...
  document.getElementById('quiz-mechanism').value = '';
