...
<label for="preset-drug-dropdown" style="font-weight: bold; color: #495057; margin-bottom: 6px;">Select Antibiotic Preset:</label>
<select id="preset-drug-dropdown" style="width: 100%; padding: 8px; border: 1px solid #ced4da; border-radius: 4px; background-color: white;">
...
::: control-group-compact
<label for="tpsa-slider">TPSA (Å²):</label> 
<input type="range" id="tpsa-slider" min="0" max="200" value="75" step="5"> 
[75]{#tpsa-value}
:::
::::::::
::::::::
...
::: control-group-compact
<label for="enteric-ph-slider">Enteric pH:</label>
<input type="range" id="enteric-ph-slider" min="6.5" max="7.5" value="7.2" step="0.1">
[7.2]{#enteric-ph-value}
:::
...
::::: calculation-results
::: {#logd-results}
<div style="background-color: #f8f9fa; padding: 8px; border-radius: 8px; border: 1px solid #dee2e6;">
<h4 style="margin-top: 0; color: #495057;">logD Values</h4>
<p><strong>Stomach:</strong> <span id="logd-stomach">-</span></p>
<p><strong>Enteric (Small Intestine):</strong> <span id="logd-enteric">-</span></p>
...
<div style="background-color: #f8f9fa; padding: 8px; border-radius: 8px; border: 1px solid #dee2e6; margin-top: 10px;">
<h4 style="margin-top: 0; color: #495057;">Ionization (%)</h4>
<p><strong>Stomach:</strong> <span id="ionized-stomach">-</span>%</p>
<p><strong>Enteric (Small Intestine):</strong> <span id="ionized-enteric">-</span>%</p>
...
<button id="download-logd-btn" onclick="downloadLogDPlot()" style="background-color: #007bff; color: white; border: none; border-radius: 4px; padding: 6px 12px; cursor: pointer;">
Download SVG
</button>
</div>
...
<div style="text-align: center; margin-bottom: 10px;">
<button id="download-ionization-btn" onclick="downloadIonizationPlot()" style="background-color: #007bff; color: white; border: none; border-radius: 4px; padding: 6px 12px; cursor: pointer;">
Download SVG
</button>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
  const presetDropdown = document.getElementById('preset-drug-dropdown');
  if (presetDropdown) {
...
let currentLogD = {
  ...
  drugName: '',
  tpsa: 75 // Topological Polar Surface Area in Ų
};
...
let pHEnvironments = {
  ...
  blood: 7.4, 
  tissue: 7.0,
  enteric: 7.2, // Small intestine pH for proper absorption modeling
  urine: 6.0
};

// Drug presets with realistic antibiotic values
const drugPresets = {
  cefepime: {
    name: 'Cefepime',
    logP: -1.02,
...
function loadPreset(drugKey) {
  ...
  document.getElementById('tpsa-slider').value = preset.tpsa;
...
// Assess permeability based on logD and TPSA
function assessPermeability(logD, tpsa = null) {
  let permeabilityScore = 0;
  
  // LogD contribution to permeability
  if (logD < -1) permeabilityScore += 1;
  else if (logD < 0) permeabilityScore += 2;
  else if (logD < 2) permeabilityScore += 3;
  else if (logD < 4) permeabilityScore += 4;
  else permeabilityScore += 5;
  
  // TPSA contribution (if provided)
  if (tpsa !== null) {
    if (tpsa > 140) permeabilityScore -= 2; // High TPSA reduces permeability
    else if (tpsa > 90) permeabilityScore -= 1; // Moderate TPSA slightly reduces permeability
    // Low TPSA (<90) doesn't modify the score
  }
  
  // Ensure score stays within bounds
  permeabilityScore = Math.max(1, Math.min(5, permeabilityScore));
  
  const permeabilityLabels = ['Very Low', 'Low', 'Moderate', 'High', 'Very High'];
  return permeabilityLabels[permeabilityScore - 1];
}
...
function calculateAndDisplayLogD() {
  ...
  document.getElementById('logd-enteric').textContent = logDValues.enteric.toFixed(2);
  ...
  document.getElementById('ionized-enteric').textContent = ionizedPercentages.enteric.toFixed(1);
  ...
  // Update permeability assessments (using TPSA for enhanced accuracy)
  const tpsa = currentLogD.tpsa;
  document.getElementById('gastric-permeability').textContent = assessPermeability(logDValues.stomach, tpsa);
  // Use proper enteric pH for intestinal absorption instead of tissue pH
  document.getElementById('intestinal-permeability').textContent = assessPermeability(logDValues.enteric, tpsa);
  document.getElementById('bbb-permeability').textContent = assessPermeability(logDValues.blood, tpsa);
...
function resetLogDSimulation() {
  ...
  document.getElementById('tpsa-slider').value = 75;
  ...
  document.getElementById('enteric-ph-slider').value = 7.2;
...
function updateLogDSliderValues() {
  const updates = [
    ...
    { slider: 'tpsa-slider', display: 'tpsa-value' },
    ...
    { slider: 'enteric-ph-slider', display: 'enteric-ph-value' },
...
// Download logD vs pH plot as SVG
function downloadLogDPlot() {
  const canvas = document.getElementById('logd-chart');
  if (!canvas) return;
  ...
  link.download = `logd_vs_ph_plot_${new Date().toISOString().split('T')[0]}.svg`;
...
// Download ionization vs pH plot as SVG
function downloadIonizationPlot() {
  const canvas = document.getElementById('ionization-chart');
  if (!canvas) return;
  ...
  const link = document.createElement('a');
  link.href = URL.createObjectURL(svgBlob);
  link.download = `ionization_vs_ph_plot_${new Date().toISOString().split('T')[0]}.svg`;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const sliderIds = [
    'logp-slider', 'pka-slider', 'tpsa-slider',
    'stomach-ph-slider', 'blood-ph-slider', 'tissue-ph-slider', 'enteric-ph-slider', 'urine-ph-slider'
  ];
  ...
  sliderIds.forEach(id => {
    ...
    if (element) {
      element.addEventListener('input', function() {
        ...
        } else if (id === 'tpsa-slider') {
          currentLogD.tpsa = parseFloat(this.value);
...
<div>
[💊 Multiple Dosing PK/PD Exercise →](exercise.qmd){.btn .btn-outline-primary}
</div>
:::::