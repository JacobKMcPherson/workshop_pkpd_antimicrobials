...
function checkQuizAnswers() {
  ...
  const ibwUsed = Math.max(ibwRaw, IBW_FLOOR_KG); // Apply instructional IBW floor.
  const ibwWorked = `\\(\\text{IBW} = \\max(${IBW_FLOOR_KG}, ${ibwBase.toFixed(1)} + 2.3 \\times (${heightInches.toFixed(1)} - 60)) = ${ibwUsed.toFixed(1)}\\)`;
  ...
  if (window.MathJax?.typesetPromise) {
    const mathJaxWarning = 'MathJax failed to render quiz feedback equations; '
      + 'LaTeX source will appear instead of formatted formulas.';
