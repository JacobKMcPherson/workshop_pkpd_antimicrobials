...
function calculateConcentration(subject, time) {
  const { clearance, vssUnbound, dose, bioavailability, interval, numDoses } = subject;
  ...
  for (let n = 0; n < numDoses; n++) {
    ...
    if (time >= doseTime) {
      ...
      // One-compartment model with first-order absorption using unbound volume
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const sliderIds = [
    ...
    // Dosing parameters
    'dose-slider', 'interval-slider', 'duration-slider', 'formulation-select',
    // Reference lines
