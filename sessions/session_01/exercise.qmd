...
function checkQuizAnswers() {
  ...
  // Steady state evaluated from 216-240 hours (days 9-10 of the simulation).
  ...
  const ibwUsed = Math.max(ibwRaw, 40);
  ...
  const volumePerKg = subject.weight > 0 ? subject.volume / subject.weight : null;
  const vssWorked = subject.weight > 0
    ? `Worked: \\(V = ${volumePerKg.toFixed(2)} \\times ${subject.weight.toFixed(1)} = ${subject.volume.toFixed(1)},\\ V_{ss,u} = ${subject.fractionUnbound.toFixed(2)} \\times ${subject.volume.toFixed(1)} = ${correctVss.toFixed(1)}\\)`
    : 'Worked: weight unavailable for Vss,u calculation.';
  updateQuizFeedback(
    vssFeedback,
  ...
  const cssAvgWorked = cssCount > 0
    ...
    : `Worked (${cssRangeLabel}): no concentrations available for averaging.`;
  updateQuizFeedback(
    cssAvgFeedback,
  ...
  if (window.MathJax?.typesetPromise) {
    MathJax.typesetPromise([crClFeedback, adjBWFeedback, vssFeedback, tssFeedback, cssAvgFeedback])
      .catch(err => console.warn('MathJax failed to render quiz feedback equations; formulas will appear as plain text.', err));
  }
}
