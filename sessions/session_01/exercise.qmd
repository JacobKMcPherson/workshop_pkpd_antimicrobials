...
This interactive module demonstrates the pharmacokinetics of multiple dosing using patient-specific parameters. You can explore how drug clearance calculated from creatinine clearance (Cockcroft-Gault equation) and weight-based volume of distribution affect drug concentration over time. The simulation updates in real-time as you adjust parameters.

## Controls

::: {.pk-controls}

::: {.patient-and-pk-section}
::: {.patient-section}
...
<input type="range" id="volume-slider" min="0.1" max="2.0" value="0.7" step="0.1">
...
#pk-table thead, #pk-table tbody, #pk-table tr {
  display: table;
  width: 100%;
  table-layout: fixed;
}
...
function generateRandomPatient() {
  ...
  const creatinine = (Math.random() * (2.0 - 0.5) + 0.5).toFixed(2);
...
function drawChart() {
  ...
  // Log scale y-axis range: 0.1 to 100 mg/L
  const minConc = 0.1;
  const maxConc = 100;
  
  // Helper functions for log scale
  const logScale = (value) => Math.log10(Math.max(value, minConc));
  const logToY = (logValue) => margin.top + height - ((logValue - logScale(minConc)) / (logScale(maxConc) - logScale(minConc))) * height;
  ...
  if (toxicityThreshold > 0 && toxicityThreshold >= minConc && toxicityThreshold <= maxConc) {
    const toxicityY = logToY(logScale(toxicityThreshold));
  ...
  if (efficacyThreshold > 0 && efficacyThreshold >= minConc && efficacyThreshold <= maxConc) {
    const efficacyY = logToY(logScale(efficacyThreshold));
  ...
  subjects.forEach((subject, subjectIndex) => {
    ...
    for (let i = 0; i < timePoints.length; i++) {
      ...
      const y = logToY(logScale(subject.concentrations[i]));
  ...
  ctx.textAlign = 'center';
  ctx.fillText('Concentration (mg/L, log scale)', 0, 0);
  ctx.restore();
  
  // Title
  ctx.textAlign = 'center';
  ctx.font = 'bold 16px Arial';
  ctx.fillText('Plasma Concentration-Time Profile (Log Scale, Blood Samples Every Hour)', canvas.width/2, 25);
  ...
  ctx.textAlign = 'right';
  const yAxisTicks = [0.1, 0.2, 0.5, 1, 2, 5, 10, 20, 50, 100];
...
function randomizeParameters() {
  ...
  // Handle interval dropdown separately - pick from available options
  const intervalOptions = [1, 2, 4, 6, 8, 12, 18, 24, 36, 48];
  const randomInterval = intervalOptions[Math.floor(Math.random() * intervalOptions.length)];
  const intervalElement = document.getElementById('interval-slider');
  if (intervalElement) {
    intervalElement.value = randomInterval;
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const sliderIds = [
    // Core PK parameters
    'f-renal-slider', 'volume-slider', 'bioavailability-slider',
    // Dosing parameters
