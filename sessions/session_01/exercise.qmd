...
- **IBW Floor**: For instructional consistency, the ideal body weight is not allowed below 40 kg
...
function checkQuizAnswers() {
  ...
  const ibwUsed = Math.max(ibwRaw, 40); // Apply clinical minimum IBW of 40 kg.
  ...
  const vssWorked = subject.weight > 0
    ...
    : 'Worked: \\(\\text{Weight unavailable for } V_{ss,u} \\text{ calculation.}\\)';
  updateQuizFeedback(
    vssFeedback,
  ...
  const cssAvgWorked = cssCount > 0
    ...
    : `Worked (${cssRangeLabel}): \\(\\text{No concentrations available for averaging.}\\)`;
  updateQuizFeedback(
    cssAvgFeedback,
