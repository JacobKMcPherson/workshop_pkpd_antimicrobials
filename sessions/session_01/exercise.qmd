...
<button id="popout-plot-btn" onclick="popOutPlot()">Pop Out Plot</button>
</div>
...
- Central compartment: dA1/dt = -k10·A1 - k12·A1 + k21·A2 + ka·Absorption
- Peripheral compartment: dA2/dt = k12·A1 - k21·A2
...
- **Clearance (CL/F)**: Dose/AUC for extravascular administration
...
- Saturable elimination: dC/dt = -Vmax × C / (Km + C)
...
let popoutWindow = null; // Track chart pop-out window
let lastPopoutSync = 0; // Limit pop-out redraw frequency
const POPOUT_SYNC_INTERVAL_MS = 150;
const POPOUT_INITIAL_SYNC_DELAY_MS = 50;
const POPOUT_MAX_SYNC_RETRIES = 5;
...
function popOutPlot() {
  ...
  if (isPopoutWindowAvailable()) {
    syncPopoutPlot(true);
    popoutWindow.focus();
    return;
  }

  popoutWindow = window.open('', 'pkPlotWindow', 'width=1000,height=700,resizable=yes,scrollbars=yes');
  if (!popoutWindow) {
    alert('Unable to open pop-out window. Please allow pop-ups for this site.');
    return;
  }
  ...
  popoutWindow.document.write(`
    ...
    <html lang="en">
      <head>
        <meta charset="utf-8">
        <title>PK Simulation Plot</title>
        <style>
          body {
            margin: 0;
            padding: 16px;
      ...
      <body>
        <h1>Plasma Concentration-Time Profile</h1>
        <img id="popout-plot-image" alt="PK simulation plot">
      </body>
    </html>
  `);
  popoutWindow.document.close();
  setTimeout(() => syncPopoutPlot(true), POPOUT_INITIAL_SYNC_DELAY_MS);
  popoutWindow.focus();
}

// Safely determine whether the pop-out window can be accessed
function isPopoutWindowAvailable() {
  if (!popoutWindow) return false;
  try {
    return !popoutWindow.closed && popoutWindow.document !== null;
  } catch (error) {
    popoutWindow = null;
    return false;
  }
...
// Keep pop-out chart synchronized with the in-page chart
function syncPopoutPlot(force = false, retryCount = 0) {
  if (!canvas || !isPopoutWindowAvailable()) return;

  const now = Date.now();
  if (!force && (now - lastPopoutSync) < POPOUT_SYNC_INTERVAL_MS) return;
  ...
  if (!force && popoutDocument.hidden === true) return;

  const popoutImage = popoutDocument.getElementById('popout-plot-image');
  if (popoutImage) {
    lastPopoutSync = now;
    popoutImage.src = canvas.toDataURL('image/png');
  } else if (force && retryCount < POPOUT_MAX_SYNC_RETRIES) {
    setTimeout(() => syncPopoutPlot(true, retryCount + 1), POPOUT_INITIAL_SYNC_DELAY_MS);
  }
}
